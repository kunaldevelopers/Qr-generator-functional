  ...
  const generateQR = async () => {
    try {
      ...
      if (security.isPasswordProtected) {
        const trimmedPassword = security.password?.trim() || "";
        if (trimmedPassword === "") {
          toast.error("Please enter a password for password-protected QR code");
          return;
        }
      ...
      setQrImage(url);
      setText(qrContent); // Set text for backward compatibility      // Save to backend
      if (user?.userId) {
        // Make sure password is properly trimmed before sending to backend
        const securitySettings = {
          ...security,
          password: security.isPasswordProtected
